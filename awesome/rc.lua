...
widget_fg_end = beautiful.widget_fg_end and beautiful.widget_fg_end or beautiful.fg_urgent
widget_border = "#282c28"
...
wicked.register(date_widget, "date", "%a %d %b - %H:%M ")

-- CPU
cpufreq_widget = widget({
    type = "textbox",
    name = "cpufreq_widget",
    align = "right"
})
...
wicked.register(cpufreq_widget, "function", function (widgets, args)
    ...
    return string.format("%.2fGHz ", l / 1000)
end, 5)

cpugraph_widget = widget({
    type = "graph",
    name = "cpugraph_widget",
    align = "right"
})

cpugraph_widget.width = "35"
cpugraph_widget.height = "0.8"
cpugraph_widget.grow = "right"
cpugraph_widget.bg = widget_bg
cpugraph_widget.border_color = widget_border
cpugraph_widget.max_value = "100"
cpugraph_widget:plot_properties_set("cpu", {
   fg = widget_fg,
...
wifi_widget_text = widget({
    type = "textbox",
    name = "wifi_widget_text",
    align = "right"
})

wicked.register(wifi_widget_text, wicked.widgets.net, "${" .. wifi_interface .. " down} ", 2, nil, 3)
...
wifi_widget.width = "35"
wifi_widget.height = "0.6"
wifi_widget.ticks_count = "10"
wifi_widget:bar_properties_set("link", {
...
wicked.register(wifi_widget, 'function', function (widgets, args)
    local cmd = "cat /proc/net/wireless"
    local f = io.open("/proc/net/wireless")

    for line in f:lines() do
       if string.find(line, wifi_interface) then
          if string.find(line, "%d*%d*%d%.") then
			 wifi_widget_text.visible = true
			 wifi_widget.visible = true
			 wifi_spacer.visible = true

             local link = string.sub(line, string.find(line, "%d*%d*%d%."))
             wifi_widget:bar_data_add("link", string.sub(link, 1, -2))
          else
             --wifi_widget:bar_data_add("link", 0)
			 wifi_widget_text.visible = false
			 wifi_widget.visible = false
			 wifi_spacer.visible = false
          end
       end
    end
...
wifi_spacer.text = " " .. spacer

-- Battery
battery_widget_text = widget({
    type = "textbox",
    name = "battery_widget_text",
    align = "right"
})
